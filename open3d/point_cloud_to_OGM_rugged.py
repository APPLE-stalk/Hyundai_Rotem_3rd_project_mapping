...
from scipy import ndimage # 기울기 mask에 사용

def build_top_height_map(voxels, origin, voxel_size, nx, nz):
    """
    voxels: Open3D VoxelGrid.get_voxels() 결과 (grid_index 속성)
    origin: (ox, oy, oz)  – VoxelGrid.origin
    nx, nz: x,z 방향 셀 수
    """
    H = np.full((nx, nz), np.nan, dtype=np.float32)   # nan → 미관측
    for v in voxels:
        ix, iz, iy = v.grid_index        # Open3D 순서 (x,z,y)
        world_y = iy * voxel_size + origin[2]
        if np.isnan(H[ix, iz]) or world_y > H[ix, iz]:   # “가장 높은 y”
            H[ix, iz] = world_y
    return H

def slope_mask(H, h, theta_deg, step):
    """
    H   : (nx, nz) Height-Map (nan 허용)
    h   : 수평 해상도 (voxel_size)
    θ_t : 임계각[deg]
    """
    # 결측 → 보간 전  패널티용 큰 음수로 임시 치환
    H_filled = np.where(np.isnan(H), np.nanmin(H) + 0.0, H)
    
    gx = ndimage.sobel(H_filled, axis=0, mode='nearest') / (16*h)
    gz = ndimage.sobel(H_filled, axis=1, mode='nearest') / (16*h)
    
    slope = np.arctan(np.sqrt(gx**2 + gz**2))     # [rad]
    
    return slope > np.deg2rad(theta_deg)
    
    
def build_ogm_top(voxels, origin, voxel_size, nx, nz,
                theta_deg=15.0, dilate_iter=1):
    H     = build_top_height_map(voxels, origin, voxel_size, nx, nz)
    steep = slope_mask(H, voxel_size, theta_deg, 7)
    ogm   = np.where(np.isnan(H), 1, 0)         # nan → 장애
    ogm[steep] = 1

    # 선택: 전차 반경만큼 팽창해 안전 마진 확보
    # if dilate_iter > 0:
    #     from scipy.ndimage import binary_dilation
    #     ogm = binary_dilation(ogm, iterations=dilate_iter).astype(np.uint8)
...
# lidar_data_path = '../hi3'
lidar_data_path = 'C:/Users/pizza/Desktop/flat_and_hills_whole'
...
# voxel grid 저장
# o3d.io.write_voxel_grid("flat_and_hills_whole_voxel_grid(0.5_0.5).ply", voxel_grid)
...
# np.save("maze 어쩌고 .npy", occ)         # .npy 형식으로 저장
...
# np.save("flat_and_hills_whole_OGM(0.5_0.5).npy", occ)         # .npy 형식으로 저장
# np.savez('flat_and_hills_whole_OGM(0.5_0.5)_with_meta.npz',
#         data       = occ,
#         origin     = voxel_grid.origin,
#         resolution = voxel_size)
